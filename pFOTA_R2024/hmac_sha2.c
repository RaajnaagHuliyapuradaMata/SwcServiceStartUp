...
#include <stdio.h>
#include <stdlib.h>
static const uint8 UDSSSAKEY[16] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                    0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F};
int main(void){
  uint8 i;
  char seed[17];
  char key[17];
  unsigned short maxSizeKey = 16;
  unsigned short sizeSeed = 16;

  for(i=0; i<16; i++){
    seed[i] = i;
  }
  hmac_sha256(UDSSSAKEY, maxSizeKey, (const uint8*)seed, sizeSeed, (uint8*)key, maxSizeKey);
  seed[16] = '\0';
  key[16] = '\0';
  while(1);
}
