...
tFblBmHdrCheckBmHeaderResult FblBmHdrVerifyBmHeader(tFblBmHdrTargetHandle targetHandle,
   ...
   if(kFblOk != ApplFblSbGetBmHeaderMac(&pBlockInfo->logicalBlock, bmHeaderMac, bmHeaderMacLength)){
      result = FBLBMHDR_CHKHDR_GET_MAC_FAIL;
   }

   if(result == FBLBMHDR_CHKHDR_OK){
      macId = FblBmHdr_GetFblSbMacId(targetHandle);
      if(kFblOk != FblSb_MacVerify(macId, (vuint32)&pBlockInfo->bmHeader, sizeof(tFblBmHdrHeader), bmHeaderMac, bmHeaderMacLength))
      {
