...
#pragma ghs section rodata=".roFBL_METADATA"
...
void VERSION_SyncPartnumber_315_433_from_Flash_to_Flash(void){
  uint8 ucCustomerPartNumber[20];
  uint8 ucRivianSpecificPartNumberString[50];
  uint8 ucSize;
  uint8 i;
  boolean bNumbersAreDifferent = FALSE;
  uint32 ulGenealogyCrc32;

  ucSize = PRODFLASH_GetCustomerEcuPartNumber(ucCustomerPartNumber);
  PRODFLASH_GetRivianEcuPartNumber(ucRivianSpecificPartNumberString);

  for (i=0; i<ucSize; i++){
    if(ucCustomerPartNumber[i] != ucRivianSpecificPartNumberString[16+i]){
      bNumbersAreDifferent = TRUE;
      break;
    }
  }

  if(bNumbersAreDifferent == TRUE){
    MEMCPY(&ucRivianSpecificPartNumberString[16], ucCustomerPartNumber, ucSize);

    PRODFLASH_PutRivianEcuPartnumber((const uint8*)ucRivianSpecificPartNumberString);
...
uint8 VERSION_GetHmacSignature(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_HASH){
    ...
    MEMCPY(pBuffer, (const uint8*)&(pDescBlock->ucHash), cSTRING_SIZ_HASH);
...
uint8 VERSION_GetPcbaId(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_PCBA_ID){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(pBuffer, (const uint8*)&(pDescBlock->ucPcbaId), cSTRING_SIZ_PCBA_ID);
...
uint8 VERSION_GetAuxId(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_AUX_ID){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(pBuffer, (const uint8*)&(pDescBlock->ucAuxId), cSTRING_SIZ_AUX_ID);
...
uint8 VERSION_GetModeId(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_MODE_ID){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(pBuffer, (const uint8*)&(pDescBlock->ushModeId), cSTRING_SIZ_MODE_ID);
...
uint8 VERSION_GetGenealogyVersion(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_SCHEMA_VERS){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(&pBuffer[0], (const uint8*)&(pDescBlock->ucSchemaVersion), cSTRING_SIZ_SCHEMA_VERS);
...
uint8 VERSION_GetTfsChangeset(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_TFSCS){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(&pBuffer[0], (const uint8*)&(pDescBlock->aucTfsChangeset), cSTRING_SIZ_TFSCS);
...
static uint8 VERSION_GetCrc32(uint8* pBuffer, uint8 ucLen, uint8 ucApplFbl){
  ...
  if(ucLen >= cSTRING_SIZ_CRC32){
    pDescBlock = VERSION_GetPointerToDescriptorBlock(ucApplFbl);
    MEMCPY(&pBuffer[0], (const uint8*)&(pDescBlock->ulCrc32App), cSTRING_SIZ_CRC32);
    return ucLen;
  }
  else{
    return 0;
